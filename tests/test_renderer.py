...
def test_markdown_renderer_initialization():
...
def test_render_successful(mock_github_api):
...
def test_render_without_token(mock_github_api):
...
@patch("httpx.post")
def test_render_unauthorized(mock_post):
...
@patch("httpx.post")
def test_render_with_retry(mock_post):
    ...
    config = TILConfig(github_token="test_token", max_retries=3, retry_delay=0.1)
    renderer = MarkdownRenderer(config)
...
@patch("httpx.post")
def test_render_max_retries_exceeded(mock_post):
...
@patch("httpx.post")
def test_render_request_error(mock_post):
    """Test handling of request errors."""
    # Mock request error
    mock_post.side_effect = httpx.RequestError("Connection error")
    
    config = TILConfig(github_token="test_token", max_retries=2, retry_delay=0.1)
    renderer = MarkdownRenderer(config)
...
@patch("httpx.post")
def test_render_request_error_then_success(mock_post):
