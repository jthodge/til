...
def test_til_processor_initialization():
    """Test TILProcessor initialization."""
    config = TILConfig(root_path=Path("/test"))
    
    with patch("til.processor.GitRepository") as mock_git, \
...
def test_til_processor_initialization_git_failure():
    """Test TILProcessor initialization when git fails."""
    config = TILConfig(root_path=Path("/test"))
    
    with patch("til.processor.GitRepository", side_effect=Exception("Git error")), \
         patch("til.processor.MarkdownRenderer") as mock_renderer, \
         patch("til.processor.TILDatabase") as mock_db:
...
def test_process_file_io_error(temp_dir):
...
def test_should_update_html_no_previous():
...
def test_should_update_html_different_body():
...
def test_should_update_html_no_html():
...
def test_should_update_html_no_update_needed():
...
def test_process_all_files(temp_dir):
...
def test_process_all_files_with_errors(temp_dir):
