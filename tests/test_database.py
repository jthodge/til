...
def test_til_database_initialization(temp_dir):
...
def test_upsert_record(temp_dir):
...
def test_get_previous_record(temp_dir):
...
def test_enable_search(temp_dir):
    ...
    # Verify FTS works
    results = list(til_db.db["til"].search("Content"))
    assert len(results) == 2


def test_get_all_by_topic(temp_dir):
...
def test_table_exists(temp_dir):
