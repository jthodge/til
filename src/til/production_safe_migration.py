...
class ProductionMigration:
    ...
    def migrate_timestamps_and_remove_duplicates(self, duplicates: Dict[Tuple[str, str], List[Dict]]) -> Tuple[int, int]:
        ...
        try:
            for key, entries in duplicates.items():
                ...
                if new_entry and old_entries:
                    ...
                    if oldest_created:
                        ...
                        logger.info(f"Migrating timestamp for {topic}/{slug}: {oldest_created}")
            ...
            if deletions:
                ...
                cursor.execute(f"DELETE FROM til WHERE path IN ({placeholders})", deletions)
    ...
    def validate_migration(self) -> bool:
        ...
        try:
            ...
            if missing_timestamps > 0:
                logger.warning(f"{missing_timestamps} content entries missing timestamps")
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            updates, deletions = self.migrate_timestamps_and_remove_duplicates(duplicates)
            logger.info(f"Migration complete: {updates} timestamps migrated, {deletions} entries deleted")
