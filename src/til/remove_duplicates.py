...
def remove_duplicates(db_path: pathlib.Path) -> None:
    ...
    try:
            ...
            if len(entries) > 1:
                ...
                if new_entry and old_entries:
                    ...
                    if oldest_created:
                        ...
                        logger.info(
                            f"  Updating {new_entry['path']} with timestamp from {old_entries[0]['path']}"
                        )
        ...
        if deletions:
            ...
            cursor.executemany(
                "DELETE FROM til WHERE path = ?", [(path,) for path in deletions]
            )

        conn.commit()
        logger.info(
            f"Database cleanup complete. Updated {len(updates)} entries, deleted {len(deletions)} entries."
        )
